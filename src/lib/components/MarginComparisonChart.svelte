	...
	<div class="flex items-center justify-between rounded-lg border-2 p-4 {getCompanyColors().bg}">
		<div class="flex items-center gap-3">
			{#if comparison.industryPosition === 'above'}
				<TrendingUp class="h-6 w-6 {getStatusColor()}" />
			{:else if comparison.industryPosition === 'below'}
				<TrendingDown class="h-6 w-6 {getStatusColor()}" />
			{:else}
				<Target class="h-6 w-6 {getStatusColor()}" />
			{/if}
			<div>
				<div class="text-lg font-semibold">
					Company Net Profit Margin: <span class={getCompanyColors().text}
						>{formatPercentage(comparison.yourNetMargin)}</span
					>
				</div>
				<div class="text-sm text-muted-foreground">
					{comparison.industryPosition === 'above'
						? 'Above'
						: comparison.industryPosition === 'below'
							? 'Below'
		...
		<div class="divide-y">
			{#each chartData() as item}
				<div class="flex items-center justify-between p-4 transition-colors hover:bg-muted/50">
					<div class="flex items-center gap-3">
						{#if item.type === 'yours'}
							<div class="h-3 w-3 {getCompanyColors().dot} animate-pulse rounded-full"></div>
							<div>
								<span class="font-semibold {getCompanyColors().text}">{item.name}</span>
							</div>
						{:else if item.type === 'industry'}
							<div
								class="h-3 w-3 {item.name === comparison.closestBenchmark.category
									? 'bg-primary'
									: 'bg-muted-foreground'} rounded-full"
							></div>
							<div>
								<div
									class="font-medium {item.name === comparison.closestBenchmark.category
										? 'text-primary'
										: 'text-foreground'}"
								>
									{item.name}
					...
					<div class="text-right">
						{#if item.type === 'yours'}
							<div class="font-bold {getCompanyColors().text} text-lg">
								{formatPercentage(item.min)}
							</div>
						{:else if item.type === 'industry'}
							<div class="text-lg font-semibold">
								{formatPercentage(item.min)} - {formatPercentage(item.max)}
							</div>
							<div class="text-xs text-muted-foreground">
								Avg: {formatPercentage(item.avgMargin)}
							</div>
						{/if}
					</div>
				</div>
	...
	<div class="grid grid-cols-3 gap-4">
		...
		>
			<div class="text-2xl font-bold text-amber-700 dark:text-amber-300">
				{formatPercentage(industryStats.industryLow)}
			</div>
			<div class="text-xs font-medium text-amber-600 dark:text-amber-300">Industry Low</div>
		</div>
		<div
			class="rounded-lg border border-gray-200 bg-gray-50 p-3 text-center dark:border-gray-800 dark:bg-gray-950/30"
		>
			<div class="text-2xl font-bold text-gray-900 dark:text-gray-100">
				{formatPercentage(industryStats.industryAverage)}
			</div>
			<div class="text-xs font-medium text-gray-700 dark:text-gray-300">Industry Average</div>
		</div>
		<div
			class="rounded-lg border border-blue-200 bg-blue-50 p-3 text-center dark:border-blue-800 dark:bg-blue-950/30"
		>
			<div class="text-2xl font-bold text-blue-700 dark:text-blue-300">
				{formatPercentage(industryStats.industryHigh)}
			</div>
			<div class="text-xs font-medium text-blue-600 dark:text-blue-300">Industry High</div>
		</div>
	</div>
