	...
	onMount(() => {
		if (browser) {
			if (!isCollaborationEnabled()) {
				console.log('Collaboration is disabled, redirecting to home');
				window.location.href = '/';
				return;
			}
		...
		{#if showSharedAlert}
			<div class="mb-6">
				<Alert.Root class="border-blue-200 bg-blue-50 dark:border-blue-800 dark:bg-blue-950/30">
					...
					<Alert.Description>
						Changes you make will be visible to other participants in real-time. Their edits will
						also sync to your view automatically.
					</Alert.Description>
				</Alert.Root>
			</div>
		{/if}
